...
static void choose_new_current_slab(struct slab_pointer *pool, int order)
{
        ...
        // pool->current_slab->node = pool->partial_slab_list;
        // list_del(&(pool->partial_slab_list));
        ...
        return;
        // struct slab_header* current_slab = pool->current_slab;
        // unsigned long current_free_cnt = current_slab->total_free_cnt;
        // unsigned long size = order_to_size(order);
        // unsigned long remain_size = size*current_free_cnt;
        // pool->current_slab = (void *)((unsigned long)current_slab+remain_size);
        /* BLANK END */
        /* LAB 2 TODO 2 END */
}
